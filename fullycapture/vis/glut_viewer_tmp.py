...
class GLUTviewer:
	...
	def mainloop(self):
		...
		if(self.element['mesh']):
			...
			#glDisable(GL_LIGHTING)
			#glEnable(GL_LIGHTING)
			self.render_buffers(num_faces,GL_LINE)
			glPolygonMode(GL_FRONT_AND_BACK,GL_FILL)
			#render trajectory
			start_traj = self.frameId - 20
			if start_traj<0:
				start_traj = 0
			glLineWidth(0.8)
			glBegin(GL_LINES)
			for cvid in range(6890):
				for fid in range(0,self.frameId):
					glColor4f(0.0,1.0,0.0,(fid-start_traj)*0.05)
					glVertex3f(self.meshes[fid]['vt'][cvid,0],self.meshes[fid]['vt'][cvid,1],self.meshes[fid]['vt'][cvid,2])
					glVertex3f(self.meshes[fid+1]['vt'][cvid,0],self.meshes[fid+1]['vt'][cvid,1],self.meshes[fid+1]['vt'][cvid,2])
			glEnd()
		...
		c_pcd = self.pcd[self.frameId]
		for pc in c_pcd:
			if pc[3]>0 and pc[3]<1e2:
				self.easySphere(pc[:3],15,[0.0,0.5,1.0,1.0])
