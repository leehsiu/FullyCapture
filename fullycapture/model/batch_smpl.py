...
class SmplModelLBS(object):
    def __init__(self, pkl_path=os.path.join(dir_path, 'neutral_smpl_with_cocoplus_reg.pkl'), reg_type='coco25', dtype=np.float32):
        ...
        if reg_type=='coco25':
            self.joint_regressor = np.array(dd['J_regressor_coco25'].todense().T)
        else:
            self.joint_regressor = self.J_regressor
        self.f = dd['f']

        self.nJoints = self.J_regressor.shape[1]
        self.J_template = self.joint_regressor.T.dot(self.v_template)
    def __call__(self, beta, theta, reg_type='coco25', name=None):
        ...
        # N x 24 x 3 x 3
        Rs = np.reshape(
            batch_rodrigues(np.reshape(theta, [-1, 3])), [-1, self.nJoints-1, 3, 3])
        # Ignore global rotation.
        pose_feature = np.reshape(Rs - np.eye(3),
        ...
        self.J_transformed, A = batch_global_rigid_transformation_no_root(Rs, J, self.parents)
