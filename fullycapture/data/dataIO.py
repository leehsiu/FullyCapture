...
def filepath_to_frameid(file_path):
	basename = os.path.basename(file_path)
	basename = os.path.splitext(basename)[0]
	frame_id = int(basename[-8:])
	return frame_id
...
def load_dp_vts_2d_single_person(root_path,frameId,model='SMPL'):
	view_num = 31
	if(model=='SMPL'):
		all_pts = np.zeros((view_num, 6890,3),np.float32)
	else:
		all_pts = np.zeros((view_num,18540,3),np.float32)
	for view_id in range(view_num):
		c_fileName = '00_{:02d}_{:08d}_IUV.txt'.format(view_id, frameId)
		c_filePath = os.path.join(root_path, c_fileName)
		if os.path.isfile(c_filePath):
			c_mat = np.loadtxt(c_filePath)
			c_mat_x = c_mat[:, 1] * 1.5
