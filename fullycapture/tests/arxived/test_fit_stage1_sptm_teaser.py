...
if __name__=='__main__':
	...
	joints_path = '/media/internal/domedb/170407_haggling_b3/hdPose3d_total'
	...
	for frame_id in frame_range:
		cFile = os.path.join(joints_path,'bodyTotal_{:08d}.json'.format(frame_id))
		dd = dataIO.load_total_joints_3d(cFile)[perId]
		j3d_t = np.zeros((65,4))
		j3d_t[0:25,:] = np.reshape(dd['joints25'],(-1,4))
		j3d_t[25:45,:] = np.reshape(dd['right_hand'],(-1,4))[1:,:]
		j3d_t[45:,:] = np.reshape(dd['left_hand'],(-1,4))[1:,:]
		j3d_t[j3d_t[:,3]<0.2,3] = 0
	...
	for frame_id in range(3500,3600):
		v_3d,v_weight = dataIO.load_dp_vts_3d('/home/xiul/databag/dome_sptm/170407_haggling_b3/dp_pcd_ransac',frame_id,perId)
	...
	with open('./stage0_171407_haggling_b3_p{}.pkl'.format(perId)) as fio:
		dd = pickle.load(fio)
	...
	with open('stage1_171407_haggling_b3_p{}.pkl'.format(perId),'wb') as fio:
