...
def triangulate_onepoint_naive(p2d_track,P_matrices,err_thre = 50):
    #Input Nx2 pts, N cams
    nview = p2d_track.shape[0]
    p3d = np.array([0,0,0])
    mean_err = -1
    if nview>=2:
        Amat = np.zeros((nview*2,4))
        for i in range(nview):
            Amat[2*i:2*i+2,:] = np.outer(p2d_track[i,:],P_matrices[i,2,:]) - P_matrices[i,0:2,:]
        p3d = core_method_dlt(Amat)
        rep_err = reproject_errors_multiview(p3d,P_matrices,p2d_track)
        mean_err = np.mean(rep_err)
    if mean_err>err_thre:
        mean_err = -1
    return p3d,mean_err
...
def triangulate_onepoint_RANSAC(pts,P_matrices,pts_prior):
    ...
    for _ in range(maxIter):
        ...
        if len(tst_inliers) >= minValid_num:
            ...
            cp3d_re = core_method_dlt(estAmat_re)
...
def reproject_p3d(p3d,P_matrix):
...
def reproject_errors_oneview(p3d,P_matrix,p2ds):
    p2drep = reproject_p3d(p3d,P_matrix)
