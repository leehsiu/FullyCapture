...
class TotalCLib(object):
	def __init__(self, lib_file='../../build/libtotalCops.so'):
		...
		self.lib.load_SMPLModel.argtypes = [ctypes.POINTER(ctypes.c_int), #f
											...
											ctypes.POINTER(ctypes.c_int),
											ctypes.c_int] 
		self.lib.load_SMPLModel.restype = None
		...
		self.lib.smpl_fit_total_stage1.argtypes = [ctypes.POINTER(ctypes.c_double)]*4+[ctypes.c_int]+[ctypes.c_bool]*2
		self.lib.smpl_fit_total_stage1.restype = None

	def smpl_fit_stage1(self,target_joints,betas,pose,trans,reg_type,fit_shape=True,showIter=True):
	...
	def load_SmplModel(self,modelpath):
		...
		self.lib.load_SMPLModel((ctypes.c_int*len(faces_))(*faces_),
		...
		# prior_A = covars.reshape(8,69*69).flatten().tolist()
		# prior_mu = means.flatten().tolist()
		# prior_w = weights.flatten().tolist()

		# self.lib.load_SMPLPrior((ctypes.c_double*len(prior_A))(*prior_A),
		# 						(ctypes.c_double*len(prior_mu))(*prior_mu),
		# 						(ctypes.c_double*len(prior_w))(*prior_w))
