...
class BodyRecon:
    def __init__(self):
        ...
        weight_coco25[:, 19:,:] = 0 #No foot currently
    ...
    def densify3d_adam(self,totalModel,j3d_t,v3d_t,init_param,joint_mask,verts_mask,n_iter=100,reg_type='coco25'):
        ...
        thetas_cu_zeros = torch.zeros_like(thetas_cu,dtype=torch.float32).cuda()
        betas_cu_zeros = torch.zeros_like(betas_cu,dtype=torch.float32).cuda()

        #j3d_weight_cu = torch.tensor(joint_mask).cuda()
        ...
        for i in range(n_iter):
            ...
            loss_total = loss_j3d + 0.5*loss_v3d + 1*loss_norm + 1e-2*loss_beta
            optimizer.zero_grad()	
            loss_total.backward()
            optimizer.step()
            print('loss in iter [{}/{}] : {}'.format(i,n_iter,loss_total.item()))
        ...
        for i in range(n_iter):
            ...
            loss_total = loss_j3d + 0.5*loss_v3d + 1e-2*loss_norm + 1e-2*loss_beta
            optimizer.zero_grad()	
            loss_total.backward()
            optimizer.step()
            print('loss in iter [{}/{}] : {}'.format(i,n_iter,loss_total.item()))
        ...
        for i in range(n_iter):
            ...
            loss_total = loss_j3d + 0.5*loss_v3d + 1e-4*loss_norm +1e-2*loss_beta
            optimizer.zero_grad()	
            loss_total.backward()
            optimizer.step()
            print('loss in iter [{}/{}] : {}'.format(i,n_iter,loss_total.item()))
